...
"""
    Module that contains the logic for URL generation and
    website scraping.
"""
...
def generate_vola_url(payload) -> str:
    ...
    """
        Function that generates Vola URL
    """
    ...
    if payload["dates"].get("anytime"):
        ...
        dates.get("departureFrom")
        and dates.get("departureTo") == dates.get("departureFrom")
        and dates.get("returnFrom")
        and dates.get("returnTo") == dates.get("returnFrom")
    ):
...
def parse_flight_stage(stage_element):
    ...
    """
    Helper function to parse the inner details
    of a 'Departure' or 'Return' stage.
    ...
    if not stage_element:
        return None
    stage_data = {}
    # 1. Top header: Extract just the Date
    # (e.g. "Du, 12 Apr. • Depature" -> "Du, 12 Apr.")
    ...
    stage_data['date'] = (
        raw_header.split('•')[0].strip() if '•' in raw_header
        else raw_header
    )
    ...
    if len(locations) >= 2:
        ...
        raw_t_time = (
            takeoff_node.select_one('div.stage-timeline-location__first-line')
        )
        ...
        t_time_text = raw_t_time.get_text(
            separator=" ",
            strip=True
        ) if raw_t_time else ""
        ...
        raw_l_time = (
            landing_node.select_one('div.stage-timeline-location__first-line')
        )
        ...
        # Isolate exactly the "HH:MM" time using regex
        l_time_text = raw_l_time.get_text(
            separator=" ",
            strip=True
        ) if raw_l_time else ""
...
def scrape_vola_flights(url: str) -> list:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # CRITICAL: Wait specifically for the
            # dynamically-rendered flight cards to appear
            page.wait_for_selector('div.flight-card__details', timeout=15000)

            # brief extra pause for final text node populating
            time.sleep(2)
            ...
            for details_div in flight_details_containers:
                ...
                if price_match:
                    ...
                    # if regex fails (or if the site loads in English)
                    # We also add an English fallback regex just in case
                    eng_price_match = re.search(
                        r'Price:\s*(\d+\s*€)',
                        raw_actions_text
                    )
                    if eng_price_match:
                        ...
                        flight_data['price_raw_actions_block'] = (
                            raw_actions_text
                        )
...
def extract_flights(payload):

    """
        Scraping the flights from the Vola URL.
    """

    url = generate_vola_url(payload)
    print(url)
